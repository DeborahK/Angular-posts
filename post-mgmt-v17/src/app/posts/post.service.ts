...
import { catchError, combineLatestWith, concatAll, concatMap, filter, groupBy, map, mergeMap, shareReplay, switchMap, tap, toArray } from 'rxjs/operators';
...
export class PostService {
  ...
  // Posts for category
  // Or all posts (if no category)
  // With post category id mapped to the category name
  postsForCategory$ = this.selectedCategory$.pipe(
    switchMap(categoryId => {
      if (categoryId === 0) {
        return this.allPosts$
      } else {
        return this.http.get<Post[]>(`${this.postsUrl}?categoryId=^${categoryId}$`)
      }
    }),
    combineLatestWith(this.categoryService.allCategories$),
    map(([posts, categories]) => this.mapCategories(posts, categories))
  );

  // Alternate technique, filtering using already retrieved posts
  postsForCategory2$ = combineLatest([
    this.selectedCategory$,
    this.allPosts$
  ]).pipe(
    map(([categoryId, posts]) => posts.filter(post => categoryId === 0 || post.categoryId === categoryId)),
    combineLatestWith(this.categoryService.allCategories$),
    map(([posts, categories]) => this.mapCategories(posts, categories))  
  )
